...
- There is a large test suite (365 tests), batch processing with JSON reports, policy profiles, a counterexample replay tool, and a published Docker image.
...
3. Run the tests from `packages/loophole`: `python -m pytest tests -q`. Expect **364 passed, 1 failed** (`test_lift_larger_matmul`, tracked as T2). Anything else means your environment differs.
